...
void CMeshDome::Update(void)
{
	...
	// 切り替え中に赤くする
	if (CManager::GetScene()->GetWaveManager()->IsWaveFrag() == false || CManager::GetScene()->GetWaveManager()->GetNowStage() < 2)
	{// 切り替えしてないときは抜ける
		return;
	}
	...
	D3DXCOLOR colDest = D3DXCOLOR(0.7f, 0.2f, 0.2f, 1.0f);

	// 目標の色に補正
	col += (colDest - col) * 0.01f;
