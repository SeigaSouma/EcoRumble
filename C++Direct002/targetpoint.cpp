...
void CTargetPoint::Control(void)
{
	...
	// 高さ取得
	bool bLand = false;
	pos.y = CGame::GetObject3DMesh()->GetHeight(pos, move, bLand, GetType());

	bLand = false;
	float fHeight = CGame::GetElevationOutside(CElevation::POSTYPE_UP)->GetHeight(pos, move, bLand, GetType());
	if (bLand == true)
	{
		pos.y = fHeight;
	}
	...
	bLand = false;
	fHeight = CGame::GetElevationOutside(CElevation::POSTYPE_RIGHT)->GetHeight(pos, move, bLand, GetType());
	if (bLand == true)
	{
		pos.y = fHeight;
	}
	...
	bLand = false;
	fHeight = CGame::GetElevationOutside(CElevation::POSTYPE_DOWN)->GetHeight(pos, move, bLand, GetType());
	if (bLand == true)
	{
		pos.y = fHeight;
	}
	...
	bLand = false;
	fHeight = CGame::GetElevationOutside(CElevation::POSTYPE_LEFT)->GetHeight(pos, move, bLand, GetType());
	if (bLand == true)
	{
		pos.y = fHeight;
	}
