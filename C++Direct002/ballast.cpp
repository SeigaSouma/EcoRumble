...
// ロード処理
//==========================================================================
HRESULT CBallast::Load(void)
{
	// Xファイルのデータ取得
	CXLoad *pXLoad = CScene::GetXLoad();

	for (int nCntFile = 0; nCntFile < TYPE_MAX; nCntFile++)
	{
		// Xファイルのロード
		m_nIdxXFile[nCntFile] = pXLoad->XLoad(m_pFileName[nCntFile]);

		// Xファイルのロード
		if (m_nIdxXFile[nCntFile] < 0)
		{// 失敗していたら
			return E_FAIL;
		}
	}
...
void CBallast::Update(void)
{
	...
	for (int nCntBallast = 0; nCntBallast < m_nNumAll; nCntBallast++)
	{
		...
		float PosY = pos.y;
		if (CGame::GetObject3DMesh()->IsHit(D3DXVECTOR3(pos.x, PosY, pos.z)) == true || GetPosition().y <= mylib_const::KILL_Y)
		{// 地面を下回ったら
